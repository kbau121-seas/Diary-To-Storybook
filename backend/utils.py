...
def generate_filename(n: int=6) -> str:
  ...
  return f"gen_image_{''.join([random.choice(string.hexdigits) for _ in range(n)])}.png"
...
def standard_image(images, cols=3, save_path=None):
  ...
  for i, img_wrapper in enumerate(images):
      ...
      generated_image = img_wrapper[0]  # Assuming each item in `images` is a list with one `GeneratedImage`
...
def upload_and_make_public(bucket_name, source_file_name, destination_blob_name) -> str:
    ...
    print(f"File uploaded to {destination_blob_name} and is publicly accessible at {blob.public_url}")
